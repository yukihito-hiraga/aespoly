from impl.blockcipher.protocol import BlockCipher, make_common_spec_bc

from Crypto.Util.strxor import strxor
from impl.common.gf import GF
from functools import reduce

from impl.specs import SchemeSpec


class AES128(BlockCipher):
    @staticmethod
    def rotword(w: bytes, k=1) -> bytes:
        return b"".join([w[(i + k) % len(w)].to_bytes() for i in range(len(w))])

    def keyex(self, key):
        self.w[0:15] = key
        for i in range(4, 44):
            temp = self.w[4 * (i - 1) : 4 * (i)]
            if i % 4 == 0:
                temp = strxor(self.subbytes(self.rotword(temp)), self.rcon[i // 4 - 1])
            self.w[4 * (i) : 4 * (i + 1)] = strxor(
                self.w[4 * (i - 4) : 4 * (i - 4 + 1)], temp
            )

    def roundkey(self, i):
        return self.w[16 * i : 16 * (i + 1)]

    @classmethod
    def blocksize(cls) -> int:
        return 16

    @classmethod
    def spec(cls) -> SchemeSpec:
        return make_common_spec_bc(
            "aes128",
            16,
            [
                {
                    "scheme" : "aes128",
                    "inputs": {
                        "key": "2b7e151628aed2a6abf7158809cf4f3c",
                        "pt": "3243f6a8885a308d313198a2e0370734",
                    },
                    "expected": {
                        "ct": "3925841d02dc09fbdc118597196a0b32",
                    },
                }
            ],
        )

    def __init__(self, key: bytes):
        self.rcon = [
            b"\x01\x00\x00\x00",
            b"\x02\x00\x00\x00",
            b"\x04\x00\x00\x00",
            b"\x08\x00\x00\x00",
            b"\x10\x00\x00\x00",
            b"\x20\x00\x00\x00",
            b"\x40\x00\x00\x00",
            b"\x80\x00\x00\x00",
            b"\x1b\x00\x00\x00",
            b"\x36\x00\x00\x00",
        ]
        self.field = GF(sum(1 << x for x in [8, 4, 3, 1, 0]))
        self.w = bytearray([0 for _ in range(180)])
        self.sbox = [
            
        ]
        self.invsbox = [
            
        ]
        self.keyex(key)
        self.M = [
            [
                self.field.from_int(2),
                self.field.from_int(3),
                self.field.from_int(1),
                self.field.from_int(1),
            ],
            [
                self.field.from_int(1),
                self.field.from_int(2),
                self.field.from_int(3),
                self.field.from_int(1),
            ],
            [
                self.field.from_int(1),
                self.field.from_int(1),
                self.field.from_int(2),
                self.field.from_int(3),
            ],
            [
                self.field.from_int(3),
                self.field.from_int(1),
                self.field.from_int(1),
                self.field.from_int(2),
            ],
        ]
        self.invM = [
            [
                self.field.from_int(0x0e),
                self.field.from_int(0x0b),
                self.field.from_int(0x0d),
                self.field.from_int(0x09),
            ],
            [
                self.field.from_int(0x09),
                self.field.from_int(0x0e),
                self.field.from_int(0x0b),
                self.field.from_int(0x0d),
            ],
            [
                self.field.from_int(0x0d),
                self.field.from_int(0x09),
                self.field.from_int(0x0e),
                self.field.from_int(0x0b),
            ],
            [
                self.field.from_int(0x0b),
                self.field.from_int(0x0d),
                self.field.from_int(0x09),
                self.field.from_int(0x0e),
            ],
        ]

    def subbytes(self, state: bytes):
        return b"".join([self.sbox[b].to_bytes() for b in state])

    def shiftrows(self, state: bytes):
        rs = [
            self.rotword(bytes([0, 4, 8, 12]), 0),
            self.rotword(bytes([1, 5, 9, 13]), 1),
            self.rotword(bytes([2, 6, 10, 14]), 2),
            self.rotword(bytes([3, 7, 11, 15]), 3),
        ]
        return b"".join([state[rs[i % 4][i // 4]].to_bytes() for i in range(16)])

    def mixcolumns(self, state: bytes):
        res = bytearray([0 for _ in range(16)])
        for k in range(4):
            s = state[4 * k : 4 * (k + 1)]
            ss = [
                reduce(
                    lambda s, x: s + x,
                    [self.M[i][j] * self.field.from_int(s[j]) for j in range(4)],
                ).value
                for i in range(4)
            ]
            res[4 * k : 4 * (k + 1)] = ss
        return res
    
    def invsubbytes(self, state: bytes):
        return b"".join([self.invsbox[b].to_bytes() for b in state])
    
    def invshiftrows(self, state:bytes):
        rs = [
            self.rotword(bytes([0, 4, 8, 12]), 0),
            self.rotword(bytes([1, 5, 9, 13]), -1),
            self.rotword(bytes([2, 6, 10, 14]), -2),
            self.rotword(bytes([3, 7, 11, 15]), -3),
        ]
        return b"".join([state[rs[i % 4][i // 4]].to_bytes() for i in range(16)])
    
    def invmixcolumns(self, state:bytes):
        res = bytearray([0 for _ in range(16)])
        for k in range(4):
            s = state[4 * k : 4 * (k + 1)]
            ss = [
                reduce(
                    lambda s, x: s + x,
                    [self.invM[i][j] * self.field.from_int(s[j]) for j in range(4)],
                ).value
                for i in range(4)
            ]
            res[4 * k : 4 * (k + 1)] = ss
        return res

    def print_state(self, state: bytes):
        for i in range(16):
            print("{:02x}".format(state[(i % 4) * 4 + (i // 4)]), end="")
            if i % 4 == 3:
                print()
        print()

    def round(self, state: bytes, key: bytes) -> bytes:
        state = self.subbytes(state)
        state = self.shiftrows(state)
        state = self.mixcolumns(state)
        state = strxor(state, key)
        return state
    
    def invround(self, state:bytes, key:bytes) -> bytes:
        state = self.invshiftrows(state)
        state = self.invsubbytes(state)
        state = strxor(state, key)
        state = self.invmixcolumns(state)
        return state

    def lastround(self, state: bytes, key: bytes) -> bytes:
        state = self.subbytes(state)
        state = self.shiftrows(state)
        state = strxor(state, key)
        return state
    
    def invlastround(self, state: bytes, key: bytes) -> bytes:
        state = self.invsubbytes(state)
        state = self.invshiftrows(state)
        state = strxor(state, key)
        return state

    def encrypt(self, pt: bytes):
        state = pt
        state = strxor(state, self.roundkey(0))
        for i in range(1, 10):
            state = self.round(state, self.roundkey(i))
        state = self.lastround(state, self.roundkey(10))
        return state
    
    def decrypt(self, ct: bytes) -> bytes:
        state = ct
        state = strxor(state, self.roundkey(10))
        for i in range(9, 0, -1):
            state = self.invround(state, self.roundkey(i))
        state = self.invlastround(state, self.roundkey(0))
        return state
            

    @staticmethod
    def encrypt_with_key(key: bytes, pt: bytes):
        instance = AES128(key)
        return instance.encrypt(pt)
    
    @staticmethod
    def decrypt_with_key(key: bytes, ct: bytes) -> bytes:
        return AES128(key).decrypt(ct)
