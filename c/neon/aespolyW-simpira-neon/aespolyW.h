...
static inline void simpira_em_encrypt(aespolyW_context ctx, const __m128i *in, __m128i *out)
{
	out[0] = _mm_xor_si128(in[0], ctx.key[0]);
	out[1] = _mm_xor_si128(in[1], ctx.key[1]);
	simpira_b2_128(ctx.simpira_ctx, out[0], out[1]);
	out[0] = _mm_xor_si128(out[0], ctx.key[0]);
	out[1] = _mm_xor_si128(out[1], ctx.key[1]);
}

static inline void hash_tl_ml_short(aespolyW_context ctx, bool mln,
									const uint8_t *T, size_t Tlen,
									const uint8_t *M, size_t Mlen,
									__m128i *out)
{
	tweaker_scalar(ctx, mln, T, Tlen, out);
	...
	out[0] = _mm_xor_si128(out[0], blk0);
	out[0] = polydot128(ctx.poly, out[0], ctx.htbl[0]);
	out[0] = _mm_xor_si128(out[0], blk1);
	out[0] = polydot128(ctx.poly, out[0], ctx.htbl[0]);

	out[1] = _mm_xor_si128(out[1], blk0);
	out[1] = polydot128(ctx.poly, out[1], ctx.htbl[16]);
	out[1] = _mm_xor_si128(out[1], blk1);
	out[1] = polydot128(ctx.poly, out[1], ctx.htbl[16]);
}
...
static inline void tweaker(aespolyW_context ctx, bool mln, const uint8_t *T, size_t Tlen, __m128i *state, __m128i *hash)
{
	...
	if (Tlen < 32)
	{
		...
		X[1] = polydot128(ctx.poly, X[1], ctx.htbl[16]);
		X[1] = _mm_xor_si128(X[1], blk1);
		state[1] = polydot128(ctx.poly, X[1], ctx.htbl[16]);
...
static inline void upper(aespolyW_context ctx, __m128i *state, const uint8_t *M, size_t Mlen, uint8_t *C, __m128i *hash, __m128i *sum)
{
	...
	if (Mlen < 32)
	{
		...
		X[0] = polydot128(ctx.poly, X[0], ctx.htbl[0]);
		X[0] = _mm_xor_si128(X[0], _mm_loadu_si128((__m128i *)padded + 1));
		X[0] = polydot128(ctx.poly, X[0], ctx.htbl[0]);
		X[1] = _mm_xor_si128(X[1], _mm_loadu_si128((__m128i *)padded));
		X[1] = polydot128(ctx.poly, X[1], ctx.htbl[16]);
...
static inline void upperx8(aespolyW_context ctx, __m128i *state, const uint8_t *M, size_t Mlen, uint8_t *C, __m128i *hash, __m128i *sum)
{
	...
	for (size_t i = 0; i < dx8len; i++)
	{
		...
		seq_graycode_n2x8(mask, omega, omega + 4, omegai, i);
...
static inline void middlelower(aespolyW_context ctx, __m128i *state, __m128i *S1, __m128i *S2, const uint8_t *M, size_t Mlen, uint8_t *C, __m128i *hash)
{
	...
	if (Mlen < 32)
	{
		...
		addkey256(ctx.simpira_ctx.keys, tmpsL, tmpsL);
		xorx2_1wise(dataL, tmpsL, tmpsL);

		memcpy(C, (uint8_t *)tmpsL, Mlen);
		memset(((uint8_t *)tmpsL) + Mlen, 0, 32 - Mlen);
		((uint8_t *)tmpsL)[Mlen] = 0x01;

		X[0] = _mm_xor_si128(X[0], tmpsL[0]);
		X[0] = polydot128(ctx.poly, X[0], ctx.htbl[0]);
		X[0] = _mm_xor_si128(X[0], tmpsL[1]);
		X[0] = polydot128(ctx.poly, X[0], ctx.htbl[0]);
		X[1] = _mm_xor_si128(X[1], tmpsL[0]);
		X[1] = polydot128(ctx.poly, X[1], ctx.htbl[16]);
		X[1] = _mm_xor_si128(X[1], tmpsL[1]);
		X[1] = polydot128(ctx.poly, X[1], ctx.htbl[16]);
		hash[0] = X[0];
		hash[1] = X[1];
		return;
...
static inline void init(aespolyW_context *ctx, uint8_t *key)
{
	...
	{
		mul2_xex_256(L + 2 * i, L + 2 * (i - 1));
	}
	for (size_t i = 1; i < 150; i++)
	{
		mul2_xex_256(omega + 2 * i, omega + 2 * (i - 1));
	}
	init_htbl128(ctx->htbl, ctx->poly, L);
}
...
static inline void encx4(aespolyW_context ctx, uint8_t *T, size_t Tlen, uint8_t *M, size_t Mlen, uint8_t *C)
{
	...
	if (MLRlen < 32 && Tr_len == 0)
	{
		alignas(16) __m128i ul[2];
		alignas(16) __m128i n[2];
		alignas(16) __m128i ctr[2];
		alignas(16) __m128i ks[2];
		alignas(16) __m128i vl[2];
		alignas(16) __m128i y0[2];

		hash_tl_ml_short(ctx, mln, T, Tl_len, MLR, MLRlen, ul);

		__m128i m0[2];
		m0[0] = _mm_loadu_si128((__m128i *)MN);
		m0[1] = _mm_loadu_si128((__m128i *)MN + 1);
		n[0] = _mm_xor_si128(ul[0], m0[0]);
		n[1] = _mm_xor_si128(ul[1], m0[1]);
		simpira_em_encrypt(ctx, n, n);

		ctr[0] = _mm_setr_epi32(1, 0, 0, 0);
		ctr[1] = _mm_setzero_si128();
		ks[0] = _mm_xor_si128(n[0], ctr[0]);
		ks[1] = _mm_xor_si128(n[1], ctr[1]);
		simpira_em_encrypt(ctx, ks, ks);

		uint8_t cl[32];
		_mm_storeu_si128((__m128i *)cl, ks[0]);
		_mm_storeu_si128((__m128i *)cl + 1, ks[1]);
		for (size_t i = 0; i < MLRlen; i++)
		{
			cl[i] ^= MLR[i];
		}
		memcpy(CLR, cl, MLRlen);

		hash_tl_ml_short(ctx, mln, T, Tl_len, cl, MLRlen, vl);
		simpira_em_encrypt(ctx, n, y0);
		n[0] = _mm_xor_si128(vl[0], y0[0]);
		n[1] = _mm_xor_si128(vl[1], y0[1]);
	...
	if (Tr_len > 0)
	{
		tmps[2] = phash(ctx, T + Tl_len, Tr_len);
	}
	...
	n[0] = _mm_xor_si128(n[0], ctx.key[0]);
	n[1] = _mm_xor_si128(n[1], ctx.key[1]);
