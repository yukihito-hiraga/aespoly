...
static inline void gctr128x4(aesgcm_context ctx, __m128i S, uint8_t *M, size_t Mlen, uint8_t *C)
{
	...
	{
		ctr[i] = _mm_add_epi32(S, _mm_setr_epi32(0, 0, 0, (i + 1) << 24));
	}
	...
	for (size_t i = 0; i < bx4len; i++)
	{
		...
		{
			ctr[j] = _mm_add_epi32(ctr[j], inc4);
		}
	}
	...
	if (remainder > 0)
	{
		...
		tmps[0] = aesenc128(ctr[0], ctx.aesctx.keys128);
		tmps[0] = _mm_xor_si128(lastblk, tmps[0]);
		memcpy(C + Mlen - remainder, (uint8_t *)(&tmps[0]), remainder);
	}
}
