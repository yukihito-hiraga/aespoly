...
TEST_P(SchemeTest, HCTR2_RIJNDAEL256_X4) {
    auto tc = load_case(GetParam());

    hctr2_context ctx;
    hctr2init128(&ctx, tc.K.data());

    bytes_t C(tc.P.size());
    hctr2enc128x4(ctx, tc.T.data(), tc.T.size(), tc.P.data(), tc.P.size(),
                  C.data());

    EXPECT_EQ(to_hex(C), to_hex(tc.E_C));
}

TEST_P(SchemeTest, HCTR2_RIJNDAEL256_X8) {
    auto tc = load_case(GetParam());
    ...
    bytes_t C(tc.P.size());
    hctr2enc128x8(ctx, tc.T.data(), tc.T.size(), tc.P.data(), tc.P.size(),
                  C.data());
...
INSTANTIATE_TEST_SUITE_P(
    Test, SchemeTest,
    ::testing::ValuesIn(load_jsonl("hctr2_rijndael256.jsonl")),
