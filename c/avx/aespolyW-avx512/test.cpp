...
TEST_P(SchemeTest, AESPOLYW_AES128_AVX512) {
    ...
    bytes_t C(tc.P.size());
    enc(&ctx, tc.T.data(), tc.T.size(), tc.P.data(), tc.P.size(), C.data());

    EXPECT_EQ(to_hex(C), to_hex(tc.E_C));
}

INSTANTIATE_TEST_SUITE_P(
    Test, SchemeTest, ::testing::ValuesIn(load_jsonl("aespolyW_aes128.jsonl")),
