...
inline __m512i polyreduce512(__m512i poly, __m512i x)
{
	...
	alignas(64) __m512i y0 = _mm512_shuffle_epi32(x, (_MM_PERM_ENUM)_MM_SHUFFLE(1, 0, 3, 2));
	alignas(64) __m512i y1 = _mm512_xor_si512(y0, x0);
	alignas(64) __m512i x1 = _mm512_clmulepi64_epi128(y1, poly, 0x10);
	alignas(64) __m512i y2 = _mm512_shuffle_epi32(y1, (_MM_PERM_ENUM)_MM_SHUFFLE(1, 0, 3, 2));
...
static inline void init(aespolyW_context *ctx, uint8_t *key)
{
	...
	ctx->poly = _mm512_broadcast_i64x2(ctx->poly128);
	...
	__m128i pp = _mm_slli_si128(ctx->poly_double128, 1);
	...
	{
		((__m128i *)ctx->L)[3 + i] = mul2rev_xex(pp, ((__m128i *)ctx->L)[2 + i]);
		((__m128i *)ctx->omega)[3 + i] = mul2rev_xex(pp, ((__m128i *)ctx->omega)[2 + i]);
	}
	...
	__m128i htbl128[16];
	init_htbl128(htbl128, ctx->poly128, _mm512_castsi512_si128(ctx->L[0]));
	for (size_t i = 0; i < 16; i++)
	{
		ctx->htbl[i] = _mm512_broadcast_i64x2(htbl128[i]);
	}
}
...
static inline void init_with_h(aespolyW_context *ctx, uint8_t *key, uint8_t *h)
{
	init(ctx, key);
	__m128i htbl128[16];
	init_htbl128(htbl128, ctx->poly128, _mm_loadu_si128((__m128i *)h));
	for (size_t i = 0; i < 16; i++)
	{
		ctx->htbl[i] = _mm512_broadcast_i64x2(htbl128[i]);
	}
...
static inline __m128i phash_scalar(aespolyW_context* ctx, const uint8_t *M, size_t Mlen)
{
	size_t blen = Mlen / 16;
	size_t brem = Mlen % 16;
	...
	{
		__m128i blk = _mm_loadu_si128((__m128i *)M + i);
		blk = _mm_xor_si128(blk, ((__m128i *)ctx->L)[4 + i]);
	...
	if (brem > 0)
	{
		...
		blk = _mm_xor_si128(blk, ((__m128i *)ctx->L)[4 + blen]);
		blk = aesenc128(blk, ctx->aes_ctx.keys128);
		sum = _mm_xor_si128(sum, blk);
	}
...
static inline __m128i tweaker_scalar(aespolyW_context* ctx, bool mln, const uint8_t *T, size_t Tlen)
{
	__m128i X =
		_mm_setr_epi64(_m_from_int64(16 * Tlen + 2 + (mln ? 1 : 0)),
					   _m_from_int64(0));
	X = polydot128(ctx->poly128, X, ((__m128i *)ctx->htbl)[0]);
...
static inline __m128i phash512(aespolyW_context* ctx, const uint8_t *M, size_t Mlen)
{
	size_t blen = Mlen / 64;
	size_t brem = Mlen % 64;
...
static inline void tweaker(aespolyW_context* ctx, bool mln, const uint8_t *T, size_t Tlen, __m128i *state, __m128i *hash)
{
	...
	alignas(16) __m128i Xinit128 = _mm_setr_epi64(_m_from_int64(16 * Tlen + 2 + mln), _m_from_int64(0));
	Xinit128 = polydot128(ctx->poly128, Xinit128, ((__m128i *)ctx->htbl)[0]);
	alignas(64) __m512i X = _mm512_setzero_si512();
	((__m128i *)&X)[0] = Xinit128;
	...
	for (size_t i = 0; i < d512x4len; i++)
	{
		...
		sum = _mm512_xor_si512(sum, tmpsR[0]);
		sum = _mm512_xor_si512(sum, tmpsR[1]);
		sum = _mm512_xor_si512(sum, tmpsR[2]);
		sum = _mm512_xor_si512(sum, tmpsR[3]);
	...
	for (size_t i = d512x4len * 4; i < d512len; i++)
	{
		...
		X = polydot512(ctx->poly, X, ctx->htbl[0]);
	}
	...
	for (size_t i = d512len * 4; i < dlen; i++)
	{
		...
		X128 = polydot128(ctx->poly128, tmpsL128[0], ((__m128i *)ctx->htbl)[0]);
	}
	...
	else
	{
		if (t % 2 == 0)
		{
			...
			state[0] = polydot128(ctx->poly128, X128, ((__m128i *)ctx->htbl)[0]);
		}
		else
		{
			data128[0] = _mm_loadu_si128((__m128i *)T + dlen * 2);
...
static inline void upper(aespolyW_context* ctx, __m128i state, const uint8_t *M, size_t Mlen, uint8_t *C, __m128i *hash, __m128i *sum128)
{
	size_t m = (Mlen + 15) / 16;
	...
	if (Mlen < 16)
	{
		...
		hash[0] = polydot128(ctx->poly128, hash[0], ((__m128i *)ctx->htbl)[0]);
		sum128[0] = _mm_setzero_si128();
		return;
	}
	...
	if (m % 2 == 0)
	{
		...
		X128 = _mm_xor_si128(X128, paddedblk);
		hash[0] = polydot128(ctx->poly128, X128, ((__m128i *)ctx->htbl)[0]);
	}
	else
	{
		uint8_t padded[17];
...
static inline __m128i middlelower(aespolyW_context* ctx, __m128i state, __m128i S1, __m128i S2, const uint8_t *M, size_t Mlen, uint8_t *C)
{
	...
	if (m % 2 == 0)
	{
		...
		X128 = _mm_xor_si128(X128, _mm_loadu_si128((__m128i *)hashblk));
		return polydot128(ctx->poly128, X128, ((__m128i *)ctx->htbl)[0]);
	}
	else
	{
		uint8_t padded[17];
...
static inline void enc(aespolyW_context* ctx, uint8_t *T, size_t Tlen, uint8_t *M, size_t Mlen, uint8_t *C)
{
	uint8_t *MN = M;
	uint8_t *MLR = M + 16;
	uint8_t *CN = C;
	...
	size_t MLRlen = Mlen < 16 ? 0 : Mlen - 16;
	...
	tmps[1] = phash_scalar(ctx, T + Tl_len, Tr_len);
