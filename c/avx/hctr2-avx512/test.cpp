...
TEST_P(SchemeTest, HCTR2_AES128_AVX512) {
    auto tc = load_case(GetParam());

    aes_context aesctx;
    hctr2_context ctx;
    hctr2init512(&aesctx, &ctx, tc.K.data());

    bytes_t C(tc.P.size());
    hctr2enc512(&aesctx, &ctx, tc.P.data(), tc.P.size(), tc.T.data(),
                tc.T.size(), C.data());

    EXPECT_EQ(to_hex(C), to_hex(tc.E_C));
}

TEST_P(SchemeTest, HCTR2_AES128_P_AVX512) {
    auto tc = load_case(GetParam());
    ...
    bytes_t C(tc.P.size());
    hctr2enc512p(&aesctx, &ctx, tc.P.data(), tc.P.size(), tc.T.data(),
                 tc.T.size(), C.data());
