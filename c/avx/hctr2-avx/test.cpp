...
TEST_P(SchemeTest, HCTR2_AES128) {
    auto tc = load_case(GetParam());

    aes_context aesctx;
    hctr2_context ctx;
    hctr2init128(&aesctx, &ctx, tc.K.data());

    bytes_t C(tc.P.size());
    hctr2enc128(aesctx, ctx, tc.P.data(), tc.P.size(), tc.T.data(), tc.T.size(),
                C.data());

    EXPECT_EQ(to_hex(C), to_hex(tc.E_C));
}

TEST_P(SchemeTest, HCTR2_AES128_PX4) {
    auto tc = load_case(GetParam());
    ...
    bytes_t C(tc.P.size());
    hctr2enc128p_x4(aesctx, ctx, tc.P.data(), tc.P.size(), tc.T.data(),
                    tc.T.size(), C.data());

    EXPECT_EQ(to_hex(C), to_hex(tc.E_C));
}

TEST_P(SchemeTest, HCTR2_AES128_PX8) {
    auto tc = load_case(GetParam());
    ...
    bytes_t C(tc.P.size());
    hctr2enc128p_x8(aesctx, ctx, tc.P.data(), tc.P.size(), tc.T.data(),
                    tc.T.size(), C.data());
...
INSTANTIATE_TEST_SUITE_P(
    Test, SchemeTest, ::testing::ValuesIn(load_jsonl("hctr2_aes128.jsonl")),
