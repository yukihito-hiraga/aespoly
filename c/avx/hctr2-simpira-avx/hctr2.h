...
void hctr2init128(hctr2_context *ctx, uint8_t *key)
{
    ...
    simpira_b2_128(ctx->simpira_ctx, ctx->L[0], ctx->L[1]);
    ctx->L[0] = _mm_xor_si128(ctx->L[0], ctx->key[0]);
    ctx->L[1] = _mm_xor_si128(ctx->L[1], ctx->key[1]);

    alignas(16) __m128i H[2] = {_mm_setr_epi32(0, 0, 0, 0), _mm_setr_epi32(0, 0, 0, 0)};

    H[0] = _mm_xor_si128(H[0], ctx->key[0]);
    H[1] = _mm_xor_si128(H[1], ctx->key[1]);
    simpira_b2_128(ctx->simpira_ctx, H[0], H[1]);
    H[0] = _mm_xor_si128(H[0], ctx->key[0]);
    H[1] = _mm_xor_si128(H[1], ctx->key[1]);
    init_htbl128(ctx, H);
}
...
static inline void hctr2enc128x4(hctr2_context ctx, uint8_t *T, size_t Tlen, uint8_t *P, size_t Mlen, uint8_t *C)
{
    ...
    UU[0] = _mm_xor_si128(UU[0], ctx.key[0]);
    UU[1] = _mm_xor_si128(UU[1], ctx.key[1]);
    simpira_b2_128(ctx.simpira_ctx, UU[0], UU[1]);
    UU[0] = _mm_xor_si128(UU[0], ctx.key[0]);
    UU[1] = _mm_xor_si128(UU[1], ctx.key[1]);
...
void hctr2dec128(aes_context aesctx, hctr2_context ctx, uint8_t *T, size_t Tlen, uint8_t *C, size_t Clen, uint8_t *P)
{
    ...
    MM[0] = _mm_xor_si128(MM[0], ctx.key[0]);
    MM[1] = _mm_xor_si128(MM[1], ctx.key[1]);
    simpira_b2_128(ctx.simpira_ctx, MM[0], MM[1]);
    MM[0] = _mm_xor_si128(MM[0], ctx.key[0]);
    MM[1] = _mm_xor_si128(MM[1], ctx.key[1]);
    ...
    xctrxoradd_hash128x4(ctx, state, S, V, Vlen, N, hash);
