...
TEST_P(SchemeTest, EME_RIJNDAEL256_X4) {
    auto tc = load_case(GetParam());

    eme_context ctx;
    emeinit(&ctx, tc.K.data());

    bytes_t C(tc.P.size());
    eme_x4(ctx, tc.T.data(), tc.P.data(), tc.P.size(), C.data());

    EXPECT_EQ(to_hex(C), to_hex(tc.E_C));
}

TEST_P(SchemeTest, EME_RIJNDAEL256_X8) {
    auto tc = load_case(GetParam());
    ...
    bytes_t C(tc.P.size());
    eme_x8(ctx, tc.T.data(), tc.P.data(), tc.P.size(), C.data());

    EXPECT_EQ(to_hex(C), to_hex(tc.E_C));
}

INSTANTIATE_TEST_SUITE_P(
    Test, SchemeTest, ::testing::ValuesIn(load_jsonl("eme_rijndael256.jsonl")),
