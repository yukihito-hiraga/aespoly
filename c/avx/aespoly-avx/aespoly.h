...
static inline void tweaker(aespoly_context ctx, bool mln, const uint8_t *T, size_t Tlen, __m128i *state, __m128i *hash)
{
	...
	alignas(16) __m128i X = _mm_setr_epi64(_m_from_int64(2 * Tlen + 2 + mln), _m_from_int64(0));
	...
	for (size_t i = 0; i < dx8len; i++)
	{
		...
		seq_graycode_x8(mask, ctx.L, ctx.L[1], ctx.L[8*i], i);
...
static inline void upperx8(aespoly_context ctx, __m128i state, const uint8_t *M, size_t Mlen, uint8_t *C, __m128i *hash, __m128i *sum)
{
	...
	for (size_t i = 0; i < dx8len; i++)
	{
		...
		seq_graycode_x8(mask, ctx.omega, ctx.omega[1], ctx.omega[8*i], i);
...
static inline void middle(aespoly_context ctx, __m128i S1, __m128i S2, const uint8_t *M, size_t Mlen, uint8_t *C)
{
	size_t m = (Mlen + 15) / 16;
	size_t rem = Mlen + 16 - m * 16;
	size_t dlen = MAX(m - m % 2 - 2, 0) / 2;
...
static inline __m128i lower(aespoly_context ctx, __m128i state, const uint8_t *M, size_t Mlen, uint8_t *C)
{
	size_t m = (Mlen + 15) / 16;
	size_t rem = Mlen + 16 - m * 16;
	size_t dlen = MAX(m - 1, 0) / 2;
	...
	for (size_t i = 0; i < dx8len; i++)
	{
		...
		seq_graycode_x8(mask, ctx.omega, ctx.omega[1], ctx.omega[8*i+1], i);
...
static inline __m128i middlelower(aespoly_context* ctx, __m128i state, __m128i S1, __m128i S2, const uint8_t *M, size_t Mlen, uint8_t *C)
{
	...
	else
	{
		...
		(tmpsL)[0] = aesenc128((ctr)[0], ctx->aes_ctx.keys128);
...
static inline __m128i middlelowerx8(aespoly_context* ctx, __m128i state, __m128i S1, __m128i S2, const uint8_t *M, size_t Mlen, uint8_t *C)
{
	size_t m = (Mlen + 15) / 16;
	size_t rem = Mlen + 16 - m * 16;
	size_t dlen = MAX(m - 1, 0) / 2;
	...
	for (size_t i = 0; i < dx8len; i++)
	{
		...
		seq_graycode_x8(mask, ctx->omega, ctx->omega[1], omegai, i);
...
static inline void init(aespoly_context *ctx, uint8_t *key)
{
	...
	{
		ctx->L[i] = double128(ctx->poly, ctx->L[i - 1]);
		ctx->omega[i] = double128(ctx->poly, ctx->omega[i - 1]);
	}
}
...
static inline void encx4(aespoly_context ctx, uint8_t *T, size_t Tlen, uint8_t *M, size_t Mlen, uint8_t *C)
{
	...
	tweaker(ctx, mln, T, Tlen, tmps, tmps + 1);
...
static inline void encx8(aespoly_context ctx, uint8_t *T, size_t Tlen, uint8_t *M, size_t Mlen, uint8_t *C)
{
	...
	tmps[5] = middlelowerx8(&ctx, tmps[0], tmps[2], tmps[2], CLR, MLRlen, CLR);
